 ...
 * Browser v1.0.0 keeps MCP bridge execution local sidecar/dev-capable.
 */
export async function testMcpConnection(serverName: string): Promise<boolean> {
  ...
  console.info(`[MCP] Browser v1.0.0 cannot connect to MCP server directly: ${serverName}`);
  updateMcpServerStatus(serverName, 'disconnected', 'Not connected directly in browser v1.0.0');
  return false;
}
